<template>
    <v-layout align-center justify-center>
        <v-flex>
            <v-card class="red darken-3" min-width="300">
                <v-card-title>
                    <v-layout align-center justify-center>
                ...
                <v-card-actions class="postit-close">
                    <v-avatar slot="icon" color="white" size="20" @click="clear">
            ...
            <v-card>
                <v-card-text>
                    <v-container>
                        <v-form ref="form">
                            <v-col cols="12">
                                <v-text-field type="text" v-model="description" :counter="15" :rules="descriptionRules"
                                label="Description" required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field class="pb-3" type="number" v-model="price" :counter="10" :rules="priceRules"
                ...
                <v-card-actions>
                    <v-layout align-center justify-center>
                        <v-btn text class="mt-3 ml-3" @click="clear" color="error"
                        :disabled="noDescription || noPrice">Cancel</v-btn>
                        <v-spacer></v-spacer>
                        <v-btn text class="mt-3 ml-3" color="primary" @click="addExpense" :disabled=" price <= 0 || 
                        description == '' || priceMustLess || descriptionMustLess">Create</v-btn>
                    </v-layout>
                </v-card-actions>
            </v-card>
        </v-flex>
